...
function GetProducts() {
    ...
    useEffect(() => {
        fetch(`${API_URL}/Products?${pageIndex ? `PageIndex=${pageIndex}` : ``}${pageSize ? `&PageSize=${pageSize}` : ``}${typeId ? ` &TypeId=${typeId}` : ``}${brandId ? `&BrandId=${brandId}` : ``}${search ? `&search=${search}` : ``}`)
            .then(res => res.json())
            .then(data => {
                                                        ...
                                                        return (
                                                            <tr className="hover:bg-gray-100 " key={item.id}>
                                                                ...
                                                                <td className="p-4 space-x-2 whitespace-nowrap">
                                                                    ...
                                                                    <Link id="deleteProductButton" className="inline-flex gap-1 items-center px-3 py-2  text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300">
