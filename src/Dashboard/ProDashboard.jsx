...
function ProDashboard() {
    ...
    useEffect(() => {
        fetch(`https://localhost:5001/api/Products?${pageIndex ? `PageIndex=${pageIndex}` : ``}${pageSize ? `&PageSize=${pageSize}` : ``}${typeId ? ` &TypeId=${typeId}` : ``}${brandId ? `&BrandId=${brandId}` : ``}${search ? `&search=${search}` : ``}`)
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <>
            <div className='products_data py-5'>
                <div className='container'>
                    <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl ">All products</h1>
                    <div className="items-center justify-start max-lg:flex-wrap flex md:divide-x gap-2 md:divide-gray-100">
                        <Link to={"/dashboard/addproduct"} id="createProductButton" className="text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 my-4 focus:outline-none" type="button">Add new product</Link>
                                                    ...
                                                    products.map((item) => {
                                                        return (
                                                            <tr className="hover:bg-gray-100 dark:hover:bg-gray-700" key={item.id}>
                                                                <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap ">{item.id}</td>
                                                                <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap">
                                                                    <div className="text-base font-semibold text-gray-900 ">{item.name}</div>
                                                                </td>
                                                                <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap ">
                                                                    <img src={item.pictureUrl} alt="product image" height="300" width="300" />
                                                                </td>
                                                                <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap ">{item.description}</td>
                                                                ...
                                                                <td className="p-4 space-x-2 whitespace-nowrap">
                                                                    <button type="button" id="updateProductButton" className="inline-flex gap-1 items-center px-3 py-2 text-white rounded-lg bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:ring-indigo-300 ">
                                                                        <BiEdit className='text-xl' />
                                                                        <span className='text-sm font-medium text-center'>Update</span>
                                                                    </button>
                                                                    <button type="button" id="deleteProductButton" className="inline-flex gap-1 items-center px-3 py-2  text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300">
